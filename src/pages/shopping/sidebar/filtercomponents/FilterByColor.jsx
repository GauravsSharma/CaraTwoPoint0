...
const FilterByColor = ({colors,resetValue,setColor}) => {
  return (
    ...
    {
      colors?.map((color,idx)=><Input key={idx} color={color} resetValue={resetValue} lable={color} id={Math.random()} name={"color"} onChange={setColor} value={color}/>)
    }
  </div>
  )
}
